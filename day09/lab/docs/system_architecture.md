...
Hệ thống Day 09 của nhóm dùng một `supervisor` để phân tích câu hỏi đầu vào và gắn các tín hiệu điều phối vào shared state, gồm `supervisor_route`, `route_reason`, `risk_high`, và `needs_tool`. Từ đó graph route sang một worker xử lý chính là `retrieval_worker` hoặc `policy_tool_worker`; node `human_review` vẫn tồn tại trong graph nhưng hiện không phải nhánh route mặc định của supervisor. Sau bước worker chính, pipeline luôn đi vào `synthesis_worker` để tạo câu trả lời cuối, trích nguồn và gán confidence.

Thành phần chính hiện tại:

- `graph.py`: supervisor orchestrator và LangGraph state machine
- `workers/retrieval.py`: lấy evidence từ ChromaDB bằng dense/hybrid retrieval
- `workers/policy_tool.py`: phân tích policy/access rule, enrich chunks và gọi MCP tools khi cần
- `workers/synthesis.py`: tổng hợp answer, ưu tiên deterministic template cho policy domains
- `mcp_server.py`: mock MCP server với các tools `search_kb`, `get_ticket_info`, `check_access_permission`, `create_ticket`, đồng thời hỗ trợ local `dispatch_tool()`
...
worker      worker                 (edge dự phòng)
   ...
   │           │    gọi MCP search_kb
   │           ├─ nếu query là multi-hop hoặc policy-specific:
   │           │    có thể enrich lại retrieved_chunks
   │           ├─ nếu là access:
   ...
   └───────────────────────┬───────────────────────┘
                 ...
                 synthesis_worker
                 - build context
                 - deterministic template cho refund/access/incident_access
                 - fallback sang grounded LLM cho domain khác
                 - cite sources
                 - estimate confidence
                           │
                           ▼
...
- `policy_tool_worker` có thể tự gọi `search_kb` qua MCP nếu chưa nhận được `retrieved_chunks`, và cũng có thể enrich thêm chunks cho query multi-hop hoặc policy-specific.
- `search_kb` trong `mcp_server.py` dùng cùng retrieval path với `workers/retrieval.py`, thay vì dense-only.
- `human_review` hiện là placeholder auto-approve trong lab mode; node vẫn còn trong graph nhưng supervisor hiện không route mặc định vào nhánh này.
...
| **Routing logic** | Query multi-hop giữa access và incident/SLA → route `policy_tool_worker`; query access/security → route `policy_tool_worker`; query refund mang tính eligibility/exception → route `policy_tool_worker`; query incident/SLA factual → route `retrieval_worker`; query refund factual như `bao nhiêu ngày`, `bao lâu` → route `retrieval_worker`; nếu không match gì → fallback `retrieval_worker`. |
| **HITL condition** | `risk_high` vẫn được set bởi các tín hiệu như `ERR-`, `emergency`, `khẩn cấp`, `2am`, `ngoài giờ`, nhưng hiện supervisor không tự động route sang `human_review`. |
...
| **Retrieval mode** | Hỗ trợ `dense` và `hybrid`; mặc định để `auto` rồi chọn theo query. |
...
| **Runtime metadata** | Worker log thêm `retrieval_backend`, `retrieval_mode`, `search_top_k`, `db_path`, `collection` vào `worker_io_logs`. |
| **Stateless?** | Yes |
...
| **Cách hoạt động** | Nếu chưa có chunks và `needs_tool=True` thì worker tự gọi `search_kb`; sau đó detect domain lại nếu cần. Với access domain, worker parse access level rồi gọi `check_access_permission`. Với incident-access, worker có thể enrich thêm chunks và gọi `get_ticket_info` khi query có ticket/P1/Jira. |
| **Structured output chính** | `policy_result` gồm `domain`, `policy_applies`, `policy_name`, `exceptions_found`, `policy_version_note`, `explanation`, `tool_findings`. |
| **Exception cases xử lý** | `flash_sale_exception`, `digital_product_exception`, `activated_exception`, `no_emergency_bypass`, và `policy_version_note` cho các case temporal scoping trước `01/02/2026`. |
...
| **Generation strategy** | Với `refund`, `access`, `incident_access`, worker ưu tiên `deterministic_template`; các domain khác mới ưu tiên LLM rồi fallback template nếu cần. |
...
| **Grounding strategy** | Build context từ chunks + policy analysis + tool findings; với policy domains, structured policy result được ưu tiên hơn prose chunks nếu có mâu thuẫn. |
| **Abstain condition** | Nếu context thiếu, policy worker báo lỗi, hoặc phát hiện temporal scope gap thì trả lời theo hướng `Không đủ thông tin trong tài liệu nội bộ` hoặc yêu cầu xác nhận thêm policy version. |

### MCP Server (`mcp_server.py`)

| Tool | Input | Output |
| ---- | ----- | ------ |
| `search_kb` | `query`, `top_k` | `chunks`, `sources`, `total_found`, `retrieval_mode`, `search_top_k`, `collection` |
...
| `create_ticket` | `priority`, `title`, `description` | mock `ticket_id`, `priority`, `title`, `status`, `url` |

**Lưu ý runtime:**

- Worker có thể gọi MCP theo local `dispatch_tool()` hoặc qua FastAPI endpoints.
- `mcp_server.py` hiện import `fastapi` và `uvicorn` ở mức module, nên local dispatch vẫn phụ thuộc môi trường đã cài các package này.
...
- Với các câu refund/access phức tạp, deterministic synthesis giúp answer bám rule tốt hơn so với để LLM tự tổng hợp hoàn toàn.
...
4. `mcp_server.py` vẫn có phụ thuộc top-level vào `fastapi/uvicorn`, nên local dispatch chưa hoàn toàn tách khỏi runtime dependency của HTTP server.
